...
class ExamViewModel @Inject constructor(
    ...
    private val examId:Long?= savedStateHandle.get<Long>("id")
    var selectedExam by mutableStateOf<ExamWithDetails?>(null)
        private set
    init {
        viewModelScope.launch {
            if(examId!=null) {
               val fetchedExam=examRepository.getExamById(examId)
                if (fetchedExam != null) {
    ...
    val studentAnswers = mutableStateMapOf<Long, Int>()
    ...
    fun finishExam() {
        ...
        val currentStudent = authRepository.currentUser.value
